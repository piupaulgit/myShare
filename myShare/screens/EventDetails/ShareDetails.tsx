    ...
    memberArray.forEach((eachMember:IGenaricObjectForMember) => {
      if(eachMember.extra < 0 ){ 
        memberArray.forEach((mem:IGenaricObjectForMember) => {
          ...
          if(amount > 0 ){
            eachMember.detail.push(`${eachMember.name} needs to pay ${amount} to ${mem.name}`)
            mem.detail.push(`${mem.name} will get ${amount} from ${eachMember.name}`)
          }
        })
      }
    });
  ...
  return (
    <SafeAreaView>
      <ScrollView>
            ...
            <VStack>
              <Heading size="sm" mb="2">Sharing Details</Heading>
              {
                memberShareDetails.length > 0 && memberShareDetails.map((member:IGenaricObjectForMember) => {
                  return(
                    <VStack bg="white" px="4" py="2" mb="2" borderRadius="3" key={member.name}>
                      ...
                      <Button bg="dark.50" size="sm" mt="3" mb="2" alignSelf="flex-start">
                        <Link to="/Bill" style={{color: '#fff'}}>Generate Bill</Link>
                      </Button>
                    </VStack>
                  )
                })
        ...
        :
        <Text fontSize="lg" alignSelf="center" mt="10">No Expense aded till now</Text>
        }
        </Stack>
      </ScrollView>
    </SafeAreaView>
